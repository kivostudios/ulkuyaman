...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/urunler/${product.id}`} className="group block">
      <div className="relative img-hover aspect-[3/4] bg-gray-50 overflow-hidden">
        <Image
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1">
          {product.isNew && (
            <span className="bg-black text-white text-[10px] tracking-widest uppercase px-2 py-1 font-medium">
              Yeni
            </span>
          )}
          {product.originalPrice && (
            <span className="bg-red-600 text-white text-[10px] tracking-widest uppercase px-2 py-1 font-medium">
              İndirim
            </span>
          )}
          {product.isBestseller && !product.isNew && (
            <span className="bg-white text-black text-[10px] tracking-widest uppercase px-2 py-1 font-medium border border-black">
              Çok Satan
            </span>
          )}
        </div>
